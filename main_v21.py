...
COMMODITY_ETFS = ["GOLDBEES.NS","SILVERBEES.NS"]

def get_asset_type(symbol):
    if symbol in COMMODITY_FUTURES.values():
        return "Commodity"
...
def relative_strength(df):
    ...
    if len(c) < 90:
        return 0
    rs30=((c.iloc[-1]/c.iloc[-30])-1)*100
    rs90=((c.iloc[-1]/c.iloc[-90])-1)*100
    return round(rs30*0.6+rs90*0.4,1)

SCAN_UNIVERSE = NIFTY500 + list(COMMODITY_FUTURES.values()) + COMMODITY_ETFS
...
def run_full_scan():
    global _cache
    _cache.update({
        "running":True, "progress":0, "total":len(SCAN_UNIVERSE),
