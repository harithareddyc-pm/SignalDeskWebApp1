...
Hosted on Replit. Provides REST API for the frontend scanner.
"""
...
_cache = {"data": None, "timestamp": None, "running": False, "progress": 0, "total": 0}
...
# ── Strategies ─────────────────────────────────────────────────────────
def strat_ema(df):
    if len(df)<215: return 0,0,"insufficient data"
    ...
    regime="uptrend" if c>e200 else "downtrend"
    return b,s,regime

def strat_rsi(df):
    if len(df)<60: return 0,0,"insufficient data"
    c,o=df["Close"].iloc[-1],df["Open"].iloc[-1]
    ...
    regime="oversold" if rc<30 else "overbought" if rc>70 else "ranging"
    return b,s,regime

def strat_bb(df):
    if len(df)<55: return 0,0,"insufficient data"
    c,o=df["Close"].iloc[-1],df["Open"].iloc[-1]
    up,mid,lo=bb_calc(df["Close"])
    uc,lc=up.iloc[-1],lo.iloc[-1]
    mc=mid.iloc[-1]
    ...
    b=min((sq*25)+int(c>uc)*25+vp+int(c>o)*25,100)
    s=min((sq*25)+int(c<lc)*25+vp+int(c<o)*25,100)
    regime="squeeze" if sq else ("expansion" if wn>wmin+0.7*wr else "normal")
    return b,s,regime
...
def strat_fvg(df):
    ...
    e200=ema(cl,200).iloc[-1]; a=atr_calc(df).iloc[-1]
    htf=int(c>e200); buy_sc=sell_sc=0
...
def strat_macd(df):
    ...
    mlc,sc,hn,hp=ml.iloc[-1],sig.iloc[-1],hist.iloc[-1],hist.iloc[-2]
    ...
    regime="momentum_up" if b>50 else "momentum_down" if s>50 else "neutral"
    return b,s,regime
...
def scan_one(symbol):
    try:
        ...
        if ag_b>=4 and avg_b>=60 and avg_b>avg_s+15: direction="LONG"; score=round(avg_b)
        elif ag_s>=4 and avg_s>=60 and avg_s>avg_b+15: direction="SHORT"; score=round(avg_s)
        else: return None
        c=df["Close"].iloc[-1]; pc=df["Close"].iloc[-2] if len(df)>1 else c
        a=atr_calc(df).iloc[-1]
        ...
        ag=ag_b if direction=="LONG" else ag_s
        return {
...
def run_full_scan():
    global _cache
    _cache["running"]=True; _cache["progress"]=0; _cache["total"]=len(NIFTY500)
    results=[]
    ...
    _cache["running"]=False; _cache["timestamp"]=datetime.now().isoformat()
